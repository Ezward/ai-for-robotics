...
def eval(r, p):
	...
	for i in range(len(p)):  # calculate mean error
		# use modulus to handle wrap-around (toroidal) world coordinates
...
T = 10  # Number of cycles; Leave this as 10 for grading purposes.
N = 1000  # number of particles

# create the robot whose position we are estimating
myrobot = robot()

# create initially random particles
p = generate_particles(1000)

for i in range(T):
	myrobot = myrobot.move(0.1, 5.0)
	Z = myrobot.sense()
	p = move_particles(p)
	w = weigh_particles(p, Z)
