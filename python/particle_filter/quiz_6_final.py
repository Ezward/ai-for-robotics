...
#           self.steering_noise
#           self.distance_noise
	def move(self, motion, add_noise = True):  # Do not change the name of this function
		...
		d = motion[1]               # distance back wheel travels

		if abs(steering_angle > max_steering_angle):
			raise ValueError("steering angle out of range")
		if d < 0:
			raise ValueError("motion cannot be reverse")
...
#           self.bearing_noise
	def sense(self, add_noise = 1):  # do not change the name of this function
		...
		def landmarkBearing(landmark):
			noise = random.gauss(0.0, self.bearing_noise) if (0 != add_noise) else 0
			z = (atan2(landmark[0] - self.y, landmark[1] - self.x) - self.orientation + noise) % (2 * pi)
			return z

		Z = list(map(landmarkBearing, landmarks))
